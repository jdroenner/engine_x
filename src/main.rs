...
/*
trait AutoGenerateDynBinaryCombos: MetaRasterOperator {
    fn auto_create_u8_raster_op<'a, O, T2>(&self) -> Box<dyn RasterSource<RasterType = u8> + 'a>
    where
        O: 'a
            + RasterSource<RasterType = u8>
            + CreateBynaryOperator<Box<dyn RasterSource<RasterType = u8>>, Box<dyn RasterSource<RasterType = T2>,
    {
        println!("MetaPlusOneOperator: create_u8_raster_op");
        let s = self.raster_sources()[0]
            .create_raster_op()
