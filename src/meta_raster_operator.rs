...
trait AutoGenerateDynBinaryCombos: MetaRasterOperator {
    fn auto_create_u8_raster_op<O, S1, S2>(
        &self,
    ) -> Box<dyn RasterSource<RasterType = u8> + 'static>
    where
        O: CreateBinaryOperator<S1, S2, String> + RasterSource<RasterType = u8> + 'static,
    {
        println!("MetaPlusOneOperator: create_u8_raster_op");
